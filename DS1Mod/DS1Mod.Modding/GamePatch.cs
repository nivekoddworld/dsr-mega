...
///
/// Construct it from a DS1Mod <c>IPatchContext</c>:
/// <code>
///   var g = new GamePatch(ctx.GameDir, ctx.BackupFile, Log);
/// </code>
/// or standalone with any game directory + a backup delegate.
///
...
public sealed class GamePatch
{
    ...
    public bool EditParams(byte[] paramdefBnd, Action<ParamRepository> edit)
    {
        ...
        byte[] dec = DCX.Decompress(path, out DCX.Type type);
        BND3 bnd = BND3.Read(dec);
        edit(new ParamRepository(bnd, ParamRepository.LoadDefs(paramdefBnd)));
        File.WriteAllBytes(path, DCX.Compress(bnd.Write(), type));
        return true;
    }
}
