...
function Ghost_Female268000Battle_Activate(ai, goal)
    ...
    if DoObserveRegion == 1 then
        if not (ai:IsInsideTargetRegion(TARGET_SELF, ObserveRegion01) or ai:IsInsideTargetRegion(TARGET_SELF, ObserveRegion02)) then
            actPerArr[8] = 100
            Kisou = 1
        elseif not (ai:IsInsideTargetRegion(TARGET_ENE_0, ObserveRegion01) or ai:IsInsideTargetRegion(TARGET_ENE_0, ObserveRegion02)) then
            actPerArr[8] = 100
            Kisou = 1
        end
    end
    ...
    local f1_local0 = {0, 60, 10, 30, 0, 0}
    local atkAfterFunc = REGIST_FUNC(ai, goal, HumanCommon_ActAfter_AdjustSpace, f1_local0)
    Common_Battle_Activate(ai, goal, actPerArr, actFuncArr, atkAfterFunc, defFuncParamTbl)
    
end

function Ghost_Female268000_Act01(ai, goal, paramTbl)
    local targetDist = ai:GetDist(TARGET_ENE_0)
...
function Ghost_Female268000_Act03(ai, goal, paramTbl)
    local targetDist = ai:GetDist(TARGET_ENE_0)
...
function Ghost_Female268000_Act06(ai, goal, paramTbl)
    local targetDist = ai:GetDist(TARGET_ENE_0)
...
function Ghost_Female268000_Act07(ai, goal, paramTbl)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local fate = ai:GetRandam_Int(1, 100)
...
function Ghost_Female268000_Act08(ai, goal, paramTbl)
...
function Ghost_Female268000_Act09(ai, goal, paramTbl)
    ai:SetTimer(0, 9999)
...
function Ghost_Female268000Battle_Update(ai, goal)
    return GOAL_RESULT_Continue
    
end

function Ghost_Female268000Battle_Terminate(ai, goal)
    
end

function Ghost_Female268000Battle_Interupt(ai, goal)
    local fate = ai:GetRandam_Int(1, 100)
    local fate2 = ai:GetRandam_Int(1, 100)
