...
public partial class DebugPage : UserControl
{
    ...
    private string BuildOverview()
    {
        ...
        if (items != null)
        {
            ...
            sb.AppendLine($"  Starting items    : {items.StartingItems.Count}  [{string.Join(", ", items.StartingItems)}]");
        }
    ...
    private string BuildItems()
    {
        ...
        // Starting gear
        sb.AppendLine("\n  ── Starting gear ────────────────────────────────");
        if (items.StartingItems.Count > 0)
            foreach (var id in items.StartingItems)
    ...
    private string BuildRaw()
    {
        ...
        if (itm != null)
        {
            sb.AppendLine($"STARTING_ITEMS: {string.Join(",", itm.StartingItems)}");
