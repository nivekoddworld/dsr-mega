...
function ShinenBito_Sorcerer416000_Act01(ai, goal, paramTbl)
    ...
    if AppDist <= targetDist and (ai:GetNpcThinkParamID() ~= 416001 or ai:GetNpcThinkParamID() == 416001 and ai:IsInsideTargetRegion(TARGET_ENE_0, 1212030)) then
    ...
    elseif targetDist <= 5 then
        if GW_fate <= 80 then
            if ai:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_F, 140) then
                ...
                ai:SetNumber(0, 1)
            elseif ai:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_R, 180) then
                oal:AddSubGoal(GOAL_COMMON_SpinStep, 3, 702, TARGET_ENE_0, 0, AI_DIR_TYPE_L, 5)
            else
                goal:AddSubGoal(GOAL_COMMON_SpinStep, 3, 703, TARGET_ENE_0, 0, AI_DIR_TYPE_R, 5)
            end
        elseif fate <= 0 then
