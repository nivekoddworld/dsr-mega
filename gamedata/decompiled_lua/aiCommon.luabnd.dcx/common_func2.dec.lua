...
function NPC_Approach_Act(ai, goal, AppDist, DashDist, Odds_Guard)
    ai:EndDash()
    local Guard = -1
...
function Approach_Act(ai, goal, AppDist, DashDist, Odds_Guard)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local Walk = true
    if DashDist <= targetDist then
        Walk = false
...
function BusyApproach_Act(ai, goal, AppDist, DashDist, Odds_Guard)
    local Guard = -1
    local G_fate = ai:GetRandam_Int(1, 100)
    if G_fate <= Odds_Guard then
        Guard = 9910
...
function Approach_and_Attack_Act(ai, goal, AppDist, DashDist, Odds_Guard, AttID, AttDist, TurnTime, TurnFaceAngle)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local Walk = true
    if DashDist <= targetDist then
        Walk = false
...
function KeepDist_and_Attack_Act(ai, goal, LeaDist, AppDist, DashDist, Odds_Guard, AttID, AttDist)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local Walk = true
    if DashDist <= targetDist then
        Walk = false
...
function Approach_and_GuardBreak_Act(ai, goal, AppDist, DashDist, Odds_Guard, GBAID, GBADist, AttID, AttDist)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local Walk = true
    if DashDist <= targetDist then
        Walk = false
...
function Approach_Act(ai, goal, AppDist, DashDist, Odds_Guard, life)
    if life == nil then
        life = 10
    end
    local targetDist = ai:GetDist(TARGET_ENE_0)
...
function Approach_or_Leave_Act(ai, goal, LeaveDist, AppDist, DashDist, Odds_Guard)
    local targetDist = ai:GetDist(TARGET_ENE_0)
    local Walk = true
